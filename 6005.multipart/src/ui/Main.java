...
public class Main extends JFrame {
	 ...
	 * and controls for animating file animation streams.
	 */
	public Main() {
	 ...
	 * If it is an animation (with suffix -seq), downloads and previews
	 * one frame at a time.
	 */
	private void startDownload() {
		...
		try {
			...
			if(path.endsWith(ANI_SUFFIX)) { // note, then remove animation type
		...
		if(!isAnimation)
			...
			progressLabel.setText("Downloading animation...");
	...
	private void downloadNextFileFromAnimation() {
		try {
			byte[] data = AnimationReader.readOneFrame(multipart);
			if(data!=null)
				preview(data);
			else { // no more sub-files
				progressLabel.setText("Animation finished.");
				finishDownload();
			}
		}
		catch(IOException e) {
